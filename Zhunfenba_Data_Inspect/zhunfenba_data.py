...
import time
from Zhunfenba_Data_Inspect import ali_oss_data_inspect,up_downline_msg,update_msg
import hashlib
...
class Zhunfenba:
    ...
    def make_data(self,obj):
        ...
        return wxid_list,ghid_list,qrPicUrl_list,nickname_list,data_wxids,now_time
    ...
    def get_imgurl(self,obj,qrPicUrl_list):
        ...
        def get_ewm(url):
            ...
            for txt in txt_list:
                ...
                qrcode_imgurl_list.append(barcodeData)

        for url in qrPicUrl_list:
            get_ewm(url)
        return qrcode_imgurl_list
    ...
    def get_mad(self,ghid_list,i):
        bb = self.md5Encode(ghid_list[i].encode('utf-8')) # 必须先进行转码，否则会报错
        return bb

    def zhunfenba(self,qrcode_imgurl_list,wxid_list,ghid_list,nickname_list,data_wxids,now_time):
        # 建立数据库连接,并且获取游标
        ...
        if rows > 0:
            ...
            for config_tup in config_tups:
                ...
                sql = """select state from ads_resource WHERE json_extract(config,"$.authorizer_appid")='%s' AND pid = 3"""%json.loads(config_tup[0])['authorizer_appid']
                cursor.execute(sql)
                ...
                if json.loads(config_tup[0])['authorizer_appid'] in data_wxids:
                    if state[0] == 1:
                        ...
                        sql = """update `ads_resource` set state = 1 where json_extract(config,"$.authorizer_appid")='%s' AND pid = 3"""%json.loads(config_tup[0])['authorizer_appid']
                        cursor.execute(sql)
                        conn.commit()
                    ...
                    for data_wxid in data_wxids:
                        if data_wxid == json.loads(config_tup[0])['authorizer_appid']:
                            ...
                            # 对微信链接进行修改
                            if json.loads(config_tup[0])['authorizer_qrcode_link'] == qrcode_imgurl_list[i]:
                                print('微信链接一致,此次判断的商家为%s\n' % nickname_list[i])
                                break
                            else:
                                sql = """update ads_resource set config = json_set(config,"$.authorizer_qrcode_link",'%s') where json_extract(config,"$.authorizer_appid")='%s' AND pid = 3;""" % (
                                    qrcode_imgurl_list[i], json.loads(config_tup[0])['authorizer_appid'])
                                cursor.execute(sql)
                                conn.commit()
                                # 自动发送微信消息提醒
                                update_msg.run(nickname_list,i)
                                ali_oss_data_inspect.run(ghid_list, i, qrcode_imgurl_list)
                                break
                        else:
                            pass
                            # print('此次判断的APPID不一致')
                    ...
                    if state[0] == 1:
                        ...
                        if check_times <= 15:
                            ...
                            sql = """update `ads_resource` set state = 2 where json_extract(config,"$.authorizer_appid")='%s' AND pid=3""" % json.loads(config_tup[0])['authorizer_appid']
                            cursor.execute(sql)
                            conn.commit()
                            print('号源信息中没有查询到%s的相关数据' % json.loads(config_tup[0])['authorizer_nickname'])
                            authorizer_nickname = json.loads(config_tup[0])
            ...
            for data_wxid in data_wxids:
                ...
                if data_wxid not in config_appid_dics['appid']:
                    ...
                    sql = """INSERT INTO `ads_resource`(`pid`, `title`, `form_name`, `classify`, `config`, `state`, `creater_username`, `creater_realname`, `is_audit`, `audit_username`, `audit_realname`, `audited_at`, `created_at`, `updated_at`, `deleted_at`) VALUES (3, '%s', 'IMAGE_QRCODE_WECHAT', 1, '{\"width\": 150, \"height\": 150, \"authorizer_appid\": \"%s\", \"authorizer_nickname\": \"%s\", \"authorizer_username\": \"%s\", \"authorizer_principal\": \"准粉吧\", \"authorizer_qrcode_str\": \"\", \"authorizer_qrcode_link\": \"%s\", \"authorizer_qrcode_ticket\": \"\", \"authorizer_qrcode_img_url\": \"%s\"}', 1, 'sysadmin', 'sysadmin', 1, 'sysadmin', 'sysadmin', '%s' , '%s' , NULL , NULL);""" % (
                        ...
                        qrcode_imgurl_list[i], 'http://res.tiancan.online/'+bb+'.png', now_time, now_time)
                    print('此次插入的数据为%s\n' % nickname_list[i])
                    cursor.execute(sql)
                    conn.commit()
                    print(data_wxid)
            ...
            for data_wxid in data_wxids:
                ...
                bb = self.get_mad(ghid_list, i)
                sql = """INSERT INTO `ads_resource`(`pid`, `title`, `form_name`, `classify`, `config`, `state`, `creater_username`, `creater_realname`, `is_audit`, `audit_username`, `audit_realname`, `audited_at`, `created_at`, `updated_at`, `deleted_at`) VALUES (3, '%s', 'IMAGE_QRCODE_WECHAT', 1, '{\"width\": 150, \"height\": 150, \"authorizer_appid\": \"%s\", \"authorizer_nickname\": \"%s\", \"authorizer_username\": \"%s\", \"authorizer_principal\": \"准粉吧\", \"authorizer_qrcode_str\": \"\", \"authorizer_qrcode_link\": \"%s\", \"authorizer_qrcode_ticket\": \"\", \"authorizer_qrcode_img_url\": \"%s\"}', 1, 'sysadmin', 'sysadmin', 1, 'sysadmin', 'sysadmin', '%s' , '%s' , NULL , NULL);""" % (
                    '准粉吧' +nickname_list[i], wxid_list[i], nickname_list[i], ghid_list[i], qrcode_imgurl_list[i],
    ...
    def no_data(self):
        conn, cursor = settings.Conn()
        sql = """update `ads_resource` set state = 2 where pid=3 AND id != 2 AND id != 67;"""
        cursor.execute(sql)
        conn.commit()
        sql = """ update `zhunfenba_extra` set check_times = 16 WHERE id > 0;"""
        cursor.execute(sql)
        conn.commit()
    ...
    def run(self):
        res = self.get_data()
        res_str = str(res)
        if 'result' in res_str:
            obj = res['result']
            if obj:
                obj = res['result']['data']
                wxid_list, ghid_list, qrPicUrl_list, nickname_list, data_wxids, now_time = self.make_data(obj)
                qrcode_imgurl_list = self.get_imgurl(obj, qrPicUrl_list)
                conn, cursor = self.zhunfenba(qrcode_imgurl_list, wxid_list, ghid_list, nickname_list, data_wxids,
                                              now_time)
                self.close(conn, cursor)
            else:
                self.no_data()
