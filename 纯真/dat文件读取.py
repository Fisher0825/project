...
class CzIp:
    def __init__(self, db_file='qqwry_lastest.dat', csv_file = '场所客户端IP.csv'):
    ...
    def get_csv_data(self):
        csv_files = csv.reader(open(self.csv_file, 'r'))
        # print(csv_files)  # 可以先输出看一下该文件是什么样的类型
...
if __name__ == '__main__':
    cz = CzIp()
    ips = cz.get_csv_data()
    csvFile = open("场所客户端.csv", "w" ,newline='')  # 创建csv文件
    writer = csv.writer(csvFile)  # 创建写的对象
    writer.writerow(["site_ap_ip", " ", "address", " ", "运营商"])  # 写入列的名称
    for ip in ips:
        # print(ip[0])
        address = cz.get_addr_by_ip(ip[0])
        # print(address)
        add = address.split()
        writer.writerow([ip[0], ' ', add[0], ' ', add[1]])