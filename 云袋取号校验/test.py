...
def get_imgurl(obj,qrcode_url_list):
    # 解析二维码对应的链接并添加到所需列表中
...
#获取游标
cursor = conn.cursor()
def yundai(authorizer_qrcode_link_list,appid_list,gh_id_list,qrcode_url_list,nick_name_list,data_appids,now_time):
    ...
    if rows > 0:
        ...
        sql = """select state from ads_resource WHERE title='%s'""" % json.loads(config_tup[0])['authorizer_nickname']
        cursor.execute(sql)
                ...
                for data_appid in data_appids:
                    ...
                    if data_appid == json.loads(config_tup[0])['authorizer_appid']:
                        ...
                        if json.loads(config_tup[0])['authorizer_qrcode_link'] == authorizer_qrcode_link_list[i]:
                            ...
                            text_push_content = """%s的url链接发生了变化，已修改""" % json.loads(config_tup[0])[
                                'authorizer_nickname']
...
if __name__ == '__main__':
    obj,appid_list,gh_id_list,qrcode_url_list,nick_name_list,data_appids,now_time=get_data()
    authorizer_qrcode_link_list = get_imgurl(obj,qrcode_url_list)
    yundai(authorizer_qrcode_link_list,appid_list,gh_id_list,qrcode_url_list,nick_name_list,data_appids,now_time)